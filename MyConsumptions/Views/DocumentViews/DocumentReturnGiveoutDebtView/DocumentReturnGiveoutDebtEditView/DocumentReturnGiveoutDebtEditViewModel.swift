//
//  DocumentReturnGiveoutDebtEditViewModel.swift
//  MyConsumptions
//
...
final class DocumentReturnGiveoutDebtEditViewModel: ObservableObject {
    
    var saveCompletion: PassthroughSubject<Void, Never> = PassthroughSubject<Void, Never>()
    
    var cdDocument: CDDocumentReturnGiveoutDebt
    ...
    init(id: NSManagedObjectID? = nil, coreData: ICoreDataStack = Model.coreData) {
        ...
        if let id = id, let cdDocument = self.viewContext.objectInContext(CDDocumentReturnGiveoutDebt.self, objectID: id) {
            self.cdDocument = cdDocument
        }
        else {
            self.cdDocument = CDDocumentReturnGiveoutDebt(context: self.viewContext)
