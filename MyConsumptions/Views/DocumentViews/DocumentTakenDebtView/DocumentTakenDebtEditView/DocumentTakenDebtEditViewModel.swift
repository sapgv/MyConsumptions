//
//  DocumentTakenDebtEditViewModel.swift
//  MyConsumptions
//
...
final class DocumentTakenDebtEditViewModel: ObservableObject {
    
    var saveCompletion: PassthroughSubject<Void, Never> = PassthroughSubject<Void, Never>()
    
    var cdDocument: CDDocumentTakenDebt
    ...
    init(id: NSManagedObjectID? = nil, coreData: ICoreDataStack = Model.coreData) {
        ...
        if let id = id, let cdDocument = self.viewContext.objectInContext(CDDocumentTakenDebt.self, objectID: id) {
            self.cdDocument = cdDocument
        }
        else {
            self.cdDocument = CDDocumentTakenDebt(context: self.viewContext)
